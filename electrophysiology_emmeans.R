...
# Exclude Nav1.8 from analysis (insufficient cells and not in scope)
datos <- datos %>% filter(Canal != "Nav1.8") %>% droplevels()

# Concentration labels (map factor levels to real experimental values)
conc_labels <- c("1" = "0.075 g/L", "2" = "0.150 g/L")

cat("=== DATA LOADED ===\n")
cat("Full data:", nrow(datos), "observations (Nav1.8 excluded)\n\n")
...
fit_and_emmeans <- function(data, channel, concentration, response_var,
  ...
  # Require at least 3 cells per treatment group for the response variable
  cells_per_trat <- subset_data %>%
    group_by(Tratamiento) %>%
    summarise(n_cells = n_distinct(Celula), .groups = "drop")
  
  if (any(cells_per_trat$n_cells < 3)) {
    min_cells <- min(cells_per_trat$n_cells)
    return(list(
      model = NULL, emm = NULL, pairs = NULL,
      channel = channel, concentration = concentration,
      variable = response_var,
      error = paste0("Fewer than 3 cells in at least one treatment group (min = ",
                     min_cells, "); skipped")
    ))
  }
...
plot_emmeans <- function(result) {
  ...
  p <- ggplot(emm_df, aes(x = Voltaje, y = emmean, color = Trat, fill = Trat)) +
    ...
    scale_fill_manual(values = c("Control" = "#2166AC", "Veneno" = "#B2182B"),
                      name = "Treatment") +
    labs(
      title = paste0(result$channel, " - ", conc_labels[as.character(result$concentration)]),
...
run_emmeans_analysis <- function(data, data_name, output_suffix) {
  ...
  for (key in names(all_results)) {
    ...
    base_title <- paste0(res$channel, " | ", conc_labels[as.character(res$concentration)],
                         " | ", res$variable)
  ...
  if (nrow(all_pairs_df) > 0) {
    heatmap_data <- all_pairs_df %>%
      mutate(
        ...
        Channel_Conc = paste0(Channel, " (", conc_labels[as.character(Concentration)], ")")
      )
